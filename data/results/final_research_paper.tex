...
\begin{table}[htbp]
\centering
\caption{Settlement-window descriptives, averaged across securities and sessions. A dash marks a quantity whose input was not produced by this run.}
\label{tab:stylised}
\small
\begin{tabular}{lrrr}
\toprule
Quantity & Expiry & Control & Difference \\
\midrule
Quoted spread (bps) & \SpreadMeanBpsExpiry{} & \SpreadMeanBpsControl{} & \SpreadMeanBpsDiff{} \\
Widest spread (bps) & \SpreadMaxBpsExpiry{} & \SpreadMaxBpsControl{} & \SpreadMaxBpsDiff{} \\
Visible bid depth (shares) & \DepthBidExpiry{} & \DepthBidControl{} & \DepthBidDiff{} \\
Visible ask depth (shares) & \DepthAskExpiry{} & \DepthAskControl{} & \DepthAskDiff{} \\
Absolute book imbalance & \DepthAbsImbalanceExpiry{} & \DepthAbsImbalanceControl{} & \DepthAbsImbalanceDiff{} \\
Orders with concealed size (\%) & \IceRatioExpiry{} & \IceRatioControl{} & \IceRatioDiff{} \\
Volume held from display (\%) & \IceHiddenShareExpiry{} & \IceHiddenShareControl{} & \IceHiddenShareDiff{} \\
Cancellations per entry & \CancelRatioExpiry{} & \CancelRatioControl{} & \CancelRatioDiff{} \\
Immediate-or-cancel share (\%) & \IocRatioExpiry{} & \IocRatioControl{} & \IocRatioDiff{} \\
IOC share, final five minutes (\%) & \IocLateRatioExpiry{} & \IocLateRatioControl{} & \IocLateRatioDiff{} \\
Orders resting under one second (\%) & \LifePhantomRateExpiry{} & \LifePhantomRateControl{} & \LifePhantomRateDiff{} \\
Median price impact (bps) & \ImpactMedianBpsExpiry{} & \ImpactMedianBpsControl{} & \ImpactMedianBpsDiff{} \\
Kyle's lambda & \ImpactKyleLambdaExpiry{} & \ImpactKyleLambdaControl{} & \ImpactKyleLambdaDiff{} \\
Settlement variance rate ratio & \VolRvRatioExpiry{} & \VolRvRatioControl{} & \VolRvRatioDiff{} \\
Volume Gini across the window & \ProfileGiniExpiry{} & \ProfileGiniControl{} & \ProfileGiniDiff{} \\
Final-minute volume share (\%) & \ProfileFinalMinShareExpiry{} & \ProfileFinalMinShareControl{} & \ProfileFinalMinShareDiff{} \\
Spread-widening episodes & \ResilShocksExpiry{} & \ResilShocksControl{} & \ResilShocksDiff{} \\
Median recovery (seconds) & \ResilRecoverySecExpiry{} & \ResilRecoverySecControl{} & \ResilRecoverySecDiff{} \\
\bottomrule
\end{tabular}
\end{table}

\begin{longtable}{p{0.7cm} p{5.4cm} r r r r}
\caption{Paired tests of each hypothesis: expiry sessions against matched controls. $n$ is the number of security-session pairs. Rejection is under Benjamini-Hochberg control of the false discovery rate.}\label{tab:tests}\\
\toprule
ID & Hypothesis & $n$ & $t$ & $p$ & $d$ \\
\midrule
\endfirsthead
\toprule
ID & Hypothesis & $n$ & $t$ & $p$ & $d$ \\
...
\bottomrule
\endlastfoot
\textbf{H1} & Basis volatility higher on expiry & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H2} & Basis divergence worse for illiquid stocks & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H3}$^{\ast}$ & Proprietary desk volume share higher on expiry & \HHThreeNPairs{} & \HHThreeTestStat{} & \HHThreePValue{} & \HHThreeEffectSizeCohenD{} \\
\textbf{H4}$^{\ast}$ & Custodian trade counts shift on expiry & \HHFourNPairs{} & \HHFourTestStat{} & \HHFourPValue{} & \HHFourEffectSizeCohenD{} \\
\textbf{H5} & Algo volume share higher on expiry & \HHFiveNPairs{} & \HHFiveTestStat{} & \HHFivePValue{} & \HHFiveEffectSizeCohenD{} \\
\textbf{H6} & Algo order IOC rate higher on expiry & \HHSixNPairs{} & \HHSixTestStat{} & \HHSixPValue{} & \HHSixEffectSizeCohenD{} \\
\textbf{H7}$^{\dagger}$ & Cancel-to-entry ratio spikes on expiry & \HHSevenNPairs{} & \HHSevenTestStat{} & \HHSevenPValue{} & \HHSevenEffectSizeCohenD{} \\
\textbf{H8}$^{\ast}$ & Cancellations concentrated in prop/algo flow & \HHEightNPairs{} & \HHEightTestStat{} & \HHEightPValue{} & \HHEightEffectSizeCohenD{} \\
\textbf{H9}$^{\ast}$ & Iceberg order ratio higher on expiry & \HHNineNPairs{} & \HHNineTestStat{} & \HHNinePValue{} & \HHNineEffectSizeCohenD{} \\
\textbf{H10} & Aggressive order ratio higher on expiry & \HHOneZeroNPairs{} & \HHOneZeroTestStat{} & \HHOneZeroPValue{} & \HHOneZeroEffectSizeCohenD{} \\
\textbf{H11} & Aggressiveness accelerates in the final 5 minutes & \HHOneOneNPairs{} & \HHOneOneTestStat{} & \HHOneOnePValue{} & \HHOneOneEffectSizeCohenD{} \\
\textbf{H12}$^{\ast}$ & Bid-ask spread widens on expiry & \HHOneTwoNPairs{} & \HHOneTwoTestStat{} & \HHOneTwoPValue{} & \HHOneTwoEffectSizeCohenD{} \\
\textbf{H13} & Spread widening worse for illiquid stocks & \HHOneThreeNPairs{} & \HHOneThreeTestStat{} & \HHOneThreePValue{} & \HHOneThreeEffectSizeCohenD{} \\
\textbf{H14} & Order book depth erosion on expiry & \HHOneFourNPairs{} & \HHOneFourTestStat{} & \HHOneFourPValue{} & \HHOneFourEffectSizeCohenD{} \\
\textbf{H15} & Depth erosion is asymmetric & \HHOneFiveNPairs{} & \HHOneFiveTestStat{} & \HHOneFivePValue{} & \HHOneFiveEffectSizeCohenD{} \\
\textbf{H16} & Order flow imbalance higher on expiry & \HHOneSixNPairs{} & \HHOneSixTestStat{} & \HHOneSixPValue{} & \HHOneSixEffectSizeCohenD{} \\
\textbf{H17}$^{\ast}$ & Price impact higher on expiry & \HHOneSevenNPairs{} & \HHOneSevenTestStat{} & \HHOneSevenPValue{} & \HHOneSevenEffectSizeCohenD{} \\
\textbf{H18}$^{\dagger}$ & Book pressure persistence higher on expiry & \HHOneEightNPairs{} & \HHOneEightTestStat{} & \HHOneEightPValue{} & \HHOneEightEffectSizeCohenD{} \\
\textbf{H19} & Book pressure magnitude higher on expiry & \HHOneNineNPairs{} & \HHOneNineTestStat{} & \HHOneNinePValue{} & \HHOneNineEffectSizeCohenD{} \\
\textbf{H20} & VWAP drift direction matches roll pressure & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H21} & VWAP drift magnitude tracks roll intensity & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H22} & Book asymmetry aligns with roll direction & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H23} & Basis mispricing larger on high roll intensity & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H24}$^{\ast}$ & Settlement realised variance rate higher on expiry & \HHTwoFourNPairs{} & \HHTwoFourTestStat{} & \HHTwoFourPValue{} & \HHTwoFourEffectSizeCohenD{} \\
\textbf{H25} & Trade concentration (HHI) higher on expiry & \HHTwoFiveNPairs{} & \HHTwoFiveTestStat{} & \HHTwoFivePValue{} & \HHTwoFiveEffectSizeCohenD{} \\
\textbf{H26} & Futures returns Granger-cause cash returns on expiry & \multicolumn{4}{c}{\textit{not evaluated - inputs absent}} \\
\textbf{H27} & Amihud illiquidity uplift higher on expiry & \HHTwoSevenNPairs{} & \HHTwoSevenTestStat{} & \HHTwoSevenPValue{} & \HHTwoSevenEffectSizeCohenD{} \\
\textbf{H28}$^{\ast}$ & Phantom order rate (<1s) higher on expiry & \HHTwoEightNPairs{} & \HHTwoEightTestStat{} & \HHTwoEightPValue{} & \HHTwoEightEffectSizeCohenD{} \\
\textbf{H29} & Settlement volume Gini higher on expiry & \HHTwoNineNPairs{} & \HHTwoNineTestStat{} & \HHTwoNinePValue{} & \HHTwoNineEffectSizeCohenD{} \\
\textbf{H30}$^{\ast}$ & Post-shock recovery time differs on expiry & \HHThreeZeroNPairs{} & \HHThreeZeroTestStat{} & \HHThreeZeroPValue{} & \HHThreeZeroEffectSizeCohenD{} \\
\end{longtable}
\noindent\footnotesize $^{\ast}$ rejected at a false discovery rate of $\alpha = \TestAlpha{}$, with the effect in the direction the hypothesis states. $^{\dagger}$ rejected at the same threshold with the effect in the opposite direction: the quantity differs, but not as claimed.\normalsize

\section{Figures}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig1_vwap_basis_trajectory.png}
  \caption{Cumulative cash VWAP and cash-futures basis across the settlement window.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig3_participant_profile.png}
  \caption{Settlement-window traded volume by participant classification.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig4_algo_ioc_rate.png}
  \caption{Immediate-or-cancel submission rate by routing classification.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig5_cancellation_ratio_timeline.png}
  \caption{Cancel-to-entry ratio by minute across the settlement window.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig6_iceberg_hidden_volume.png}
  \caption{Share of entered volume held back from display, by liquidity group.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig7_spread_dynamics.png}
  \caption{Quoted spread by liquidity group, expiry against control.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig8_order_flow_imbalance.png}
  \caption{Net submitted order flow by minute across the settlement window.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig9_price_impact_bps.png}
  \caption{Median absolute trade-to-trade price change, expiry against control.}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{fig10_hypothesis_forest_plot.png}
  \caption{Paired effect sizes with 95\% intervals. Hypotheses without inputs are marked.}
\end{figure}

\section{What this design cannot establish}\label{sec:limits}
