"""Directional validation: Bloomberg roll pressure vs NSE microstructure (Stage 6 C3, H20-H23)."""
...
def run_c3_directional_validation() -> pd.DataFrame:
    print("[ANALYSIS C3] Cross-Referencing Bloomberg Roll Direction with NSE Microstructure...")

    c1_file = Path(RESULTS_DIR) / "c1_roll_pressure.csv"
    a1_file = Path(RESULTS_DIR) / "a1_vwap_trajectory.csv"
    b5_file = Path(RESULTS_DIR) / "b5_book_asymmetry.csv"
    b3_file = Path(RESULTS_DIR) / "b3_order_flow_imbalance.csv"
    out_csv = Path(RESULTS_DIR) / "c3_directional_validation.csv"
