...
def run_a5_cancellation_patterns() -> pd.DataFrame:
    ...
    query = f"""
    SELECT 
        TRIM(symbol) AS symbol, trade_date, time_bucket, is_expiry, participant_type, algo_type,
    ...
    GROUP BY TRIM(symbol), trade_date, time_bucket, is_expiry, participant_type, algo_type
