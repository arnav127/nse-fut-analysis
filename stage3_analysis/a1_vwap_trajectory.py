...
def run_a1_vwap_trajectory() -> pd.DataFrame:
    ...
    cash_files = glob.glob(f"{cash_path}/**/*.parquet", recursive=True)
    fao_files = glob.glob(f"{fao_path}/**/*.parquet", recursive=True)
    if not cash_files:
        print("[WARN] Enriched cash trades missing for A1 analysis.")
        return pd.DataFrame()
    ...
    WITH cash_min AS (
        SELECT 
            ...
            SUM(SUM(trade_price * trade_quantity)) OVER (
                PARTITION BY TRIM(symbol), trade_date 
    ...
    FROM cash_min c
    LEFT JOIN fao_min f ON c.symbol = f.symbol AND c.trade_date = f.trade_date AND c.time_bucket = f.time_bucket
