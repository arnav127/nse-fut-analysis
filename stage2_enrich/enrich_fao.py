"""
enrich_fao.py — Enrich parsed FAO orders and trades with timestamps, prices, and labels.
"""
import os
...
def enrich_fao_orders(spark=None):
    parsed_path = os.path.join(PARSED_DATA_DIR, "fao_orders")
    out_path = os.path.join(ENRICHED_DATA_DIR, "fao_orders")
    ...
    print("[ENRICH] Enriching FAO Orders...")
    df = spark.read.parquet(parsed_path)

    # Filter to Futures contracts (FF) if applicable
    df = df.filter(F.trim(F.col("instrument")) == "FUTSTK")
    ...
    df = df.withColumn("strike_price", F.col("strike_price").cast("double") / 100.0)
...
def enrich_fao_trades(spark=None):
    parsed_path = os.path.join(PARSED_DATA_DIR, "fao_trades")
    out_path = os.path.join(ENRICHED_DATA_DIR, "fao_trades")
    if not os.path.exists(parsed_path):
        print(f"[WARN] Parsed path does not exist: {parsed_path}")
        return
    ...
    print("[ENRICH] Enriching FAO Trades...")
    df = spark.read.parquet(parsed_path)
