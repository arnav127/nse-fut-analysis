 ...
 *
 * Implements a high-performance, cache-aligned limit order book using contiguous
 * memory allocations (FlatPriceBook) for ultra-low latency snapshotting and 
 * matching simulation.
 */
...
struct alignas(16) OrderInfo {
    ...
    char pad[3];       // 3 bytes explicit padding
};
#pragma pack(pop)
...
public:
    ...
    [[nodiscard]] double best_price() const noexcept { return levels.empty() ? 0.0 : levels.front().price; }
...
private:
    ...
    void add_order(long long order_number, char side, double price, long long qty) {
        ...
        orders[order_number] = OrderInfo{price, static_cast<int32_t>(qty), side, {0, 0, 0}};
    }
...
public:
    ...
    void process_event(long long order_number, int activity_type, const std::string& side, double price, long long qty) {
        const char side_char = (!side.empty() && side[0] == 'B') ? 'B' : 'S';
    ...
    py::dict snapshot(int depth = 10) {
        ...
        if (has_bid && has_ask) {
            const double midpoint = (best_bid + best_ask) * 0.5;
            const double spread = best_ask - best_bid;
            const double spread_bps = (midpoint > 0.0) ? (spread / midpoint * 10000.0) : 0.0;
    ...
    py::list process_event_stream(
        ...
        const int n = static_cast<int>(kind_buf.shape(0));
        int last_snap_sec = -1;

        for (int i = 0; i < n; ++i) {
            const int ek  = kind_buf(i);
            const int sec = sec_buf(i);

            if (ek == 1) {
                const int sc = side_buf(i);
                const std::string s = (sc == 1) ? "B" : "S";
                process_event(order_num_buf(i), act_type_buf(i), s, price_buf(i), vol_buf(i));
            } else if (ek == 2) {
                const int64_t tq = trade_qty_buf(i);
                remove_traded_qty(buy_num_buf(i), tq);
                remove_traded_qty(sell_num_buf(i), tq);
            }

            if (sec >= start_sec && sec <= end_sec) {
                const int sec_from_1500 = sec - start_sec;
                if (sec_from_1500 != last_snap_sec) {
                    ...
                    const int h = 15 + sec_from_1500 / 3600;
                    const int m = (sec_from_1500 % 3600) / 60;
                    const int s = sec_from_1500 % 60;
                    char buf[16];
