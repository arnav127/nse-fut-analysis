"""Limit Order Book implementation with automatic C++ PyBind11 and Python fallbacks."""

import os
import sys
from pathlib import Path
from typing import Any, Dict, Optional
from sortedcontainers import SortedDict

# Auto-register MinGW DLL directories on Windows if present
if sys.platform == "win32":
    known_dll_paths = [
        r"C:\msys64\ucrt64\bin",
        r"C:\msys64\mingw64\bin",
        r"C:\MinGW\bin",
    ]
    for dll_path in known_dll_paths:
        if os.path.exists(dll_path):
            try:
                os.add_dll_directory(dll_path)
            except Exception:
                pass

try:
    from order_book_cpp import OrderBookCpp as CppEngine  # type: ignore
    HAS_CPP = True
except ImportError:
    HAS_CPP = False


class OrderBookPython:
    """Pure Python fallback for Limit Order Book replay and depth snapshotting."""

    def __init__(self) -> None:
        self.bid: SortedDict = SortedDict()
        self.ask: SortedDict = SortedDict()
        self.orders: Dict[int, Dict[str, Any]] = {}

    def process_event(self, order_number: int, activity_type: int, side: str, price: float, qty: int) -> None:
        if activity_type == 1:
            self._add_order(order_number, side, price, qty)
        elif activity_type == 3:
            self._cancel_order(order_number)
        elif activity_type == 4:
            self._cancel_order(order_number)
            self._add_order(order_number, side, price, qty)

    def _add_order(self, order_number: int, side: str, price: float, qty: int) -> None:
        if price <= 0 or qty <= 0:
            return
        book = self.bid if side == "B" else self.ask
        book[price] = book.get(price, 0) + qty
        self.orders[order_number] = {"side": side, "price": price, "qty": qty}

    def _cancel_order(self, order_number: int) -> None:
        if order_number not in self.orders:
            return
        info = self.orders.pop(order_number)
        book = self.bid if info["side"] == "B" else self.ask
        if info["price"] in book:
            book[info["price"]] -= info["qty"]
            if book[info["price"]] <= 0:
                del book[info["price"]]

    def remove_traded_qty(self, order_number: int, traded_qty: int) -> None:
        if order_number not in self.orders:
            return
        info = self.orders[order_number]
        book = self.bid if info["side"] == "B" else self.ask
        info["qty"] -= traded_qty
        if info["price"] in book:
            book[info["price"]] -= traded_qty
            if book[info["price"]] <= 0:
                del book[info["price"]]
        if info["qty"] <= 0:
            del self.orders[order_number]

    def snapshot(self, depth: int = 10) -> Dict[str, Any]:
        best_bids = list(self.bid.items())[-depth:][::-1]
        best_asks = list(self.ask.items())[:depth]

        best_bid_price: Optional[float] = best_bids[0][0] if best_bids else None
        best_ask_price: Optional[float] = best_asks[0][0] if best_asks else None

        midpoint = (best_bid_price + best_ask_price) / 2.0 if (best_bid_price and best_ask_price) else None
        spread = (best_ask_price - best_bid_price) if (best_bid_price and best_ask_price) else None
        spread_bps = (spread / midpoint * 10000.0) if (spread and midpoint and midpoint > 0) else None

        tot_bid_vol = sum(q for _, q in best_bids)
        tot_ask_vol = sum(q for _, q in best_asks)
        imbalance = (tot_bid_vol - tot_ask_vol) / float(tot_bid_vol + tot_ask_vol) if (tot_bid_vol + tot_ask_vol) > 0 else 0.0

        snap: Dict[str, Any] = {
            "best_bid": best_bid_price,
            "best_ask": best_ask_price,
            "midpoint": midpoint,
            "spread": spread,
            "spread_bps": spread_bps,
            "total_bid_volume": tot_bid_vol,
            "total_ask_volume": tot_ask_vol,
            "book_imbalance": imbalance,
        }

        for i in range(1, depth + 1):
            snap[f"bid_depth_{i}"] = best_bids[i - 1][1] if i <= len(best_bids) else 0
            snap[f"ask_depth_{i}"] = best_asks[i - 1][1] if i <= len(best_asks) else 0

        return snap


class OrderBook:
    """Unified Order Book wrapper selecting C++ engine or Python fallback."""

    def __init__(self) -> None:
        if HAS_CPP:
            self._engine: Any = CppEngine()
            self.is_cpp = True
        else:
            self._engine = OrderBookPython()
            self.is_cpp = False

    def process_event(self, order_number: int, activity_type: int, side: str, price: float, qty: int) -> None:
        self._engine.process_event(order_number, activity_type, side, price, qty)

    def remove_traded_qty(self, order_number: int, traded_qty: int) -> None:
        self._engine.remove_traded_qty(order_number, traded_qty)

    def snapshot(self, depth: int = 10) -> Dict[str, Any]:
        return self._engine.snapshot(depth)
